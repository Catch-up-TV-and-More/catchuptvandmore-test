    ...
    while(Route.continue_epxloration()):
        ...
        with mock.patch('sys.argv', current_route.get_fake_sys_argv()):
            ...
            if RuntimeErrorCQ.last_menu_triggered_error:
                ...
                log.warn('')
                log.warn(WARNING + ' The last selection triggered an error (see log above) ' + WARNING)
                log.warn('')
                ...
                if len(RuntimeErrorCQ.all_errors) >= Config.get('exit_after_x_errors'):
                    log.info('Max number of error reached ({}) --> Exit'.format(Config.get('exit_after_x_errors')))
                    next_item = -1
                else:
            ...
            # Else print the current directory
                ...
                if not entry_point_reached:
                    ...
                    if next_item == 'R':
                        ...
                        log.info('Entry point not yet reached --> random next item: {}'.format(next_item))
                    else:
                        next_item = int(next_item)
                        log.info('Entry point not yet reached --> next item: {}'.format(next_item))
                ...
                # Else if we are in auto exploration
                    ...
                    log.info('next_item selected by auto exploration: {}'.format(next_item))
